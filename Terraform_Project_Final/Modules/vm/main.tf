...
resource "azurerm_network_interface_security_group_association" "web_nsg" {
  count = can(regex(".*${var.Webserver_name}.*", var.vm_name)) ? 1 : 0
...
resource "azurerm_network_interface_backend_address_pool_association" "db_pool" {
  count = can(regex(".*${var.Database_name}.*", var.vm_name)) ? 1 : 0
...
resource "azurerm_virtual_machine_extension" "mariadb_install" {
  ...
  settings = <<SETTINGS
    {
      "fileUris": ["https://${var.blob_name}.blob.core.windows.net/install/install_mariadb.sh"],
...
resource "azurerm_linux_virtual_machine" "main" {
  ...
  custom_data = can(regex(".*${var.Webserver_name}.*", var.vm_name)) ? base64encode(<<-SCRIPT
    #!/bin/bash
    ...
    PLAYBOOK_URL="https://${var.blob_name}.blob.core.windows.net/play/playbook.yml"
    PLAYBOOK_PATH="/home/${var.admin_username}/playbook.yml"

    # Download the Ansible playbook
    curl -o /home/${var.admin_username}/playbook.yml https://${var.blob_name}.blob.core.windows.net/play/playbook.yml

    # Run the playbook
    ansible-playbook /home/${var.admin_username}/playbook.yml
    SCRIPT
  ) : null
